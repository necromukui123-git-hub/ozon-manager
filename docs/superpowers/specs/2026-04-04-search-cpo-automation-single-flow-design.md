...
4. 详情中必须明确记录：`退出促销活动失败，跳过后续动作`
5. 若退出步骤成功或跳过，则执行 `enable`
6. `enable` 成功后继续执行 `Morkovsk`
...
3. 跳过重复 `enable`
4. 跳过重复加入 `Morkovsk`
5. 若退出步骤失败，则该商品整体记为失败，并在详情中明确记录：`退出促销活动失败，跳过后续动作`
...
3. 每一步的 `success / failed / skipped`
   - 若某商品在状态 2 / 3 / 4 的退出步骤失败，后续步骤必须显式返回 `skipped`
   - 跳过原因统一写入详情消息：`退出促销活动失败，跳过后续动作`
4. 已有的诊断字段
   - `requested_sku`
