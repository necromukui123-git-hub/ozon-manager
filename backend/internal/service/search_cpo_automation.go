...
const (
	...
	searchCPOLegacyRuleStateState3   = "state3_trigger"
	searchCPOLegacyRuleStateState4   = "morkovsk_joined"
)
...
func isSearchCPOJoinedRepairState(state *searchCPOAutomationItemState) bool {
	...
	return normalizeSearchCPORuleState(state.RuleStateAfter) == model.SearchCPORuleStateState4
}
...
func normalizeSearchCPORuleState(state string) string {
	...
	case searchCPOLegacyRuleStateState3:
		return model.SearchCPORuleStateState3
	case searchCPOLegacyRuleStateState4:
		return model.SearchCPORuleStateState4
	default:
...
func deriveSearchCPOLiveRuleState(product model.SearchCPOProduct, now time.Time) (string, *time.Time, bool) {
	...
	if searchStatus == "" || product.AvailabilityPromo == nil {
		return "", product.State2DetectedAt, false
	}
	if searchStatus == "SEARCH_PROMO_STATUS_DISABLED" {
	...
	if searchStatus == "SEARCH_PROMO_STATUS_ENABLED" && carrotsStatus == "CARROTS_STATUS_DISABLED" {
		if product.State2DetectedAt != nil {
			return model.SearchCPORuleStateState3, product.State2DetectedAt, true
		}
		detectedAt := now
		return model.SearchCPORuleStateState3, &detectedAt, true
	}
	if searchStatus == "SEARCH_PROMO_STATUS_ENABLED" && carrotsStatus == "CARROTS_STATUS_ENABLED" {
		return model.SearchCPORuleStateState4, product.State2DetectedAt, true
	}
	return "", product.State2DetectedAt, false
}
...
func shouldResetSearchCPOJoinedMarker(product model.SearchCPOProduct, liveState string, liveConfirmed bool) bool {
	...
	return normalizeSearchCPORuleState(liveState) != model.SearchCPORuleStateState4
}
...
func (s *SearchCPOService) GetAutomationRunDetail(shopID, runID uint) (*dto.SearchCPOAutomationRunDetailResponse, error) {
	...
	productMap, err := s.loadSearchCPOAvailabilityProducts(shopID, run.RunItems)
	if err != nil {
		return nil, err
	}
	...
	for _, item := range run.RunItems {
		product := productMap[item.SourceSKU]
		items = append(items, dto.SearchCPOAutomationRunItemResponse{
			...
			RuleStateBefore:         normalizeSearchCPORuleState(item.RuleStateBefore),
			RuleStateAfter:          normalizeSearchCPORuleState(item.RuleStateAfter),
			...
			AvailabilityCheckedAt:   formatOptionalTime(product.AvailabilityCheckedAt),
			AvailabilityDiagnostics: decodeSearchCPOAvailabilityDiagnostics(product.AvailabilityPayload),
...
func (s *SearchCPOService) loadSearchCPOAvailabilityProducts(shopID uint, runItems []model.SearchCPOAutoRunItem) (map[string]model.SearchCPOProduct, error) {
	result := make(map[string]model.SearchCPOProduct)
	sourceSKUs := make([]string, 0, len(runItems))
	for _, item := range runItems {
		sku := strings.TrimSpace(item.SourceSKU)
		if sku != "" {
			sourceSKUs = append(sourceSKUs, sku)
		}
	}
	...
	products, err := s.repo.FindProductsBySourceSKUs(shopID, sourceSKUs)
	if err != nil {
		return nil, err
	}
	for _, product := range products {
		result[strings.TrimSpace(product.SourceSKU)] = product
	}
	return result, nil
}
...
func (s *SearchCPOService) runAutomationExecution(run *model.SearchCPOAutoRun, input searchCPOAutomationRunInput) error {
	...
	state4SKUs := make([]string, 0)
	now := time.Now()
	for _, product := range products {
		before := normalizeSearchCPORuleState(product.RuleState)
		...
		case model.SearchCPORuleStateState3:
			state3SKUs = append(state3SKUs, product.SourceSKU)
		case model.SearchCPORuleStateState4:
			state4SKUs = append(state4SKUs, product.SourceSKU)
		}
	}
	...
	run.TotalState3 = len(state3SKUs)
	run.TotalState4 = len(state4SKUs)
	run.TotalProcessed = len(state1SKUs) + len(state2SKUs) + len(state3SKUs) + len(state4SKUs)
	...
	migrationSKUs := uniqueSourceSKUs(append(append(append([]string{}, state2SKUs...), state3SKUs...), state4SKUs...))
...
func (s *SearchCPOService) processMigrationItems(input searchCPOAutomationRunInput, triggerUserID uint, sourceSKUs []string, itemStates map[string]*searchCPOAutomationItemState) error {
	...
	if len(pendingEnable) > 0 {
		...
		for _, sku := range pendingEnable {
			...
			if state.RuleStateAfter == model.SearchCPORuleStateState2 {
				state.RuleStateAfter = model.SearchCPORuleStateState3
			}
	...
	for _, sku := range eligibleForMorkovsk {
		...
		if step, ok := morkovskMap[sku]; ok {
			...
			if step.Status == model.SearchCPOItemStatusSuccess {
				now := time.Now()
				state.RuleStateAfter = model.SearchCPORuleStateState4
				state.Product.SearchPromoStatus = "SEARCH_PROMO_STATUS_ENABLED"
				...
				if err := s.repo.UpdateProductFields(input.ShopID, sku, map[string]interface{}{
					...
					"rule_state":          model.SearchCPORuleStateState4,
				}); err != nil {
					return err
				}
			}
...
func deriveSearchCPORuleState(product model.SearchCPOProduct, _ string, now time.Time) (string, *time.Time) {
	if liveState, detectedAt, ok := deriveSearchCPOLiveRuleState(product, now); ok {
		return normalizeSearchCPORuleState(liveState), detectedAt
	}
	searchStatus := strings.TrimSpace(product.SearchPromoStatus)
	carrotsStatus := strings.TrimSpace(product.CarrotsStatus)
	if searchStatus == "SEARCH_PROMO_STATUS_DISABLED" && product.AvailabilityPromo != nil {
	...
	if searchStatus == "SEARCH_PROMO_STATUS_ENABLED" && product.AvailabilityPromo != nil {
		if carrotsStatus == "CARROTS_STATUS_ENABLED" {
			return model.SearchCPORuleStateState4, product.State2DetectedAt
		}
		if product.State2DetectedAt != nil && carrotsStatus == "CARROTS_STATUS_DISABLED" {
			return model.SearchCPORuleStateState3, product.State2DetectedAt
		}
		if carrotsStatus == "CARROTS_STATUS_DISABLED" {
			detectedAt := now
			return model.SearchCPORuleStateState3, &detectedAt
		}
	}
	return model.SearchCPORuleStateOther, product.State2DetectedAt
}

func summarizeSearchCPOActionResultsStatus(results []dto.SearchCPORunActionResult) string {
	if len(results) == 0 {
	...
	for _, result := range results {
		switch normalizeSearchCPOStepStatus(result.Status) {
		case model.SearchCPOItemStatusFailed:
	...
	if hasSuccess && hasSkipped {
		return model.SearchCPOItemStatusPartialSuccess
	}
	if hasSuccess {
...
func toSearchCPOAutomationRunSummaryDTO(run *model.SearchCPOAutoRun) *dto.SearchCPOAutomationRunSummaryResponse {
	...
	return &dto.SearchCPOAutomationRunSummaryResponse{
		...
		TotalState3:    run.TotalState3,
		TotalState4:    run.TotalState4,
...
func decodeSearchCPOAvailabilityDiagnostics(raw datatypes.JSON) *dto.SearchCPOAvailabilityDiagnostics {
	if len(raw) == 0 {
		return nil
	}
	var payload struct {
		RequestedSKU            string   `json:"requested_sku"`
		ParserRevision          string   `json:"parser_revision"`
		BuildRevision           string   `json:"build_revision"`
		ResponseRootKeys        []string `json:"response_root_keys"`
		SampleResponseKeys      []string `json:"sample_response_keys"`
		AvailabilityMapKeyCount int      `json:"availability_map_key_count"`
		ReasonMapKeyCount       int      `json:"reason_map_key_count"`
		ResponseHTTPStatus      int      `json:"response_http_status"`
		ResponseHTTPStatusText  string   `json:"response_http_status_text"`
		ResponseContentType     string   `json:"response_content_type"`
		ResponseParseError      string   `json:"response_parse_error"`
		ResponseExcerpt         string   `json:"response_excerpt"`
		ResponseLength          int      `json:"response_length"`
		ResponseKind            string   `json:"response_kind"`
		ScriptResultType        string   `json:"script_result_type"`
		UnavailableReason       string   `json:"unavailableReason"`
		UnavailableReasonSnake  string   `json:"unavailable_reason"`
	}
	if err := json.Unmarshal(raw, &payload); err != nil {
		return nil
	}
	diagnostics := &dto.SearchCPOAvailabilityDiagnostics{
		RequestedSKU:            strings.TrimSpace(payload.RequestedSKU),
		ParserRevision:          strings.TrimSpace(payload.ParserRevision),
		BuildRevision:           strings.TrimSpace(payload.BuildRevision),
		ResponseRootKeys:        trimSearchCPODiagnosticStrings(payload.ResponseRootKeys),
		SampleResponseKeys:      trimSearchCPODiagnosticStrings(payload.SampleResponseKeys),
		AvailabilityMapKeyCount: payload.AvailabilityMapKeyCount,
		ReasonMapKeyCount:       payload.ReasonMapKeyCount,
		ResponseHTTPStatus:      payload.ResponseHTTPStatus,
		ResponseHTTPStatusText:  strings.TrimSpace(payload.ResponseHTTPStatusText),
		ResponseContentType:     strings.TrimSpace(payload.ResponseContentType),
		ResponseParseError:      strings.TrimSpace(payload.ResponseParseError),
		ResponseExcerpt:         strings.TrimSpace(payload.ResponseExcerpt),
		ResponseLength:          payload.ResponseLength,
		ResponseKind:            strings.TrimSpace(payload.ResponseKind),
		ScriptResultType:        strings.TrimSpace(payload.ScriptResultType),
		UnavailableReason:       firstNonEmptySearchCPODiagnosticText(payload.UnavailableReason, payload.UnavailableReasonSnake),
	}
	if !hasSearchCPOAvailabilityDiagnostics(diagnostics) {
		return nil
	}
	return diagnostics
}

func trimSearchCPODiagnosticStrings(values []string) []string {
...
func firstNonEmptySearchCPODiagnosticText(values ...string) string {
	for _, value := range values {
...
func hasSearchCPOAvailabilityDiagnostics(value *dto.SearchCPOAvailabilityDiagnostics) bool {
	if value == nil {
		return false
	}
	return value.RequestedSKU != "" ||
		...
		value.AvailabilityMapKeyCount > 0 ||
		value.ReasonMapKeyCount > 0 ||
...
func summarizeSearchCPOShopExitJobItem(jobItem model.AutomationJobItem, fallback string) (string, string) {
	message := firstNonEmptyServiceTrimmed(jobItem.StepExitError, jobItem.StepReaddError, jobItem.StepRepriceError, fallback)
	if isSkippableSearchCPOExitMessage(message) {
		return model.SearchCPOItemStatusSkipped, message
	}
	switch normalizeSearchCPOStepStatus(jobItem.StepExitStatus) {
	case model.SearchCPOItemStatusSuccess:
		return model.SearchCPOItemStatusSuccess, ""
	case model.SearchCPOItemStatusSkipped:
		return model.SearchCPOItemStatusSkipped, message
	default:
		return model.SearchCPOItemStatusFailed, firstNonEmptyServiceTrimmed(message, "店铺活动退出失败")
	}
}

func isSkippableSearchCPOExitMessage(message string) bool {
	text := strings.TrimSpace(strings.ToLower(message))
	if text == "" {
		return false
	}
	if strings.Contains(text, "商品当前不在活动中") {
		return true
	}
	if strings.Contains(text, "action_not_found") {
		return true
	}
	if strings.Contains(text, "status 404") || strings.Contains(text, "404 not found") {
		return true
